...
**The tile worker.** MapLibre decodes tiles in a Web Worker whose URL it derives from its own module's location. In a Vite production bundle that file is not next to the chunk, so the style loads but no tile is ever decoded and the map stays white. `wallmap.ts` therefore hands MapLibre the worker as a Vite-bundled asset (`maplibre-gl-worker.mjs?worker&url`, with `worker.format: "es"` in `vite.config.ts`); in development the package is served as-is (`optimizeDeps.exclude`) and the same import works.
