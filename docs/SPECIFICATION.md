...
| Wand pose (6-DOF like the head) in every frame: absolute from a tracker (`setWand`), else derived from the head as a hand offset in the head's yaw frame (`defaultWand`, simulator Shift keys adjust it), drawn in the overview; CAVE <-> world helpers for apps | `src/core/protocol.ts`, `src/core/pose.ts`, `src/core/navigation.ts`, `src/simulator/main.ts`, `src/render/overview.ts` | done; tracker bridge to come |
...
| Raw WebGL app contract: render(ctx) per eye with off-axis matrices into the bound eye target, multisample resolve, state reset; "aquarium" app: WebGL Aquarium (tdl) adapted to one instance per context, cluster time, hashed effects, settings panel | `src/apps/types.ts`, `src/render/viewport.ts`, `src/apps/aquarium/`, `public/aquarium/` | done |
| WebGPU app contract: OffscreenCanvas per eye, image copied into the WebGL eye target (transferToImageBitmap + texSubImage2D), same matrices as raw apps; "metaballs" app: webgpu-metaballs driven through its XR view path, GL-to-WebGPU depth remap, style / resolution / stats panel | `src/apps/types.ts`, `src/render/viewport.ts`, `src/apps/metaballs/` | done; first WebGPU app |
| "hackernews" app: HTML content in the CAVE; controller fetches the public Hacker News API and publishes a snapshot, cards on a curved wall as CanvasTextures rendered through HTML-in-canvas (`drawElementImage`) or Canvas 2D, wand-ray pointing computed per node, comments panel, sidebar | `src/apps/hackernews/` | done; mock-up |
| Headset viewer `xr.html`: joins as a controller, renders scene and raw apps into an immersive WebXR session with the XR floor origin at the CAVE origin under the navigation transform, mono preview from the cluster head, optional headset-as-head (`setHead`) and right controller as wand (`setWand`) with buttons and sticks through the action layer; HTTPS dev server and `/manager` WebSocket proxy; WebXR polyfill for Cardboard / testing | `src/xr/main.ts`, `xr.html`, `vite.config.ts` | done; tested with the polyfill, not yet on a headset |
